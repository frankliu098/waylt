            ...
            for (const artist of song.artists) {
                ...
                .then((artist_data) => {
                    ...
                    if (last && artist == song.artists.at(-1)) {
                        console.log(applyConversion(dailyStatistics));
                    }
                });
            }
        }
...
router.get('/userInfo', (req, res, next) => {
    spotifyApi.getMe()
    .then(selfData => {
        res.send(JSON.stringify({display_name: selfData.body.display_name, email: selfData.body.email, pfp_url: selfData.body.images[0].url}))
    })
})
